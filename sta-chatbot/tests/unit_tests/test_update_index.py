...
def test_list_files(mock_fs_ls_side_effect, expected, expected_call_count):
    ...
    result = index_update.utils.list_files(mock_dbutils, '/dir')
...
@patch('index_update.utils.TextLoader')
@pytest.mark.unit
def test_extract_text_with_text_loader(mock_text_loader):
    ...
    result = index_update.utils.extract_text_with_text_loader("fake_file_path.txt")
...
@patch('index_update.utils.Docx2txtLoader')
@pytest.mark.unit
def test_extract_text_with_microsoft_word_loader(mock_docx2txt_loader):
    ...
    result = index_update.utils.extract_text_with_microsoft_word_loader("fake_file_path.docx")
...
@patch('index_update.utils.PdfReader')
@pytest.mark.unit
def test_extract_text_with_pdf_reader(mock_pdf_reader):
    ...
    result = index_update.utils.extract_text("fake_file_path.pdf")
...
def test_extract_text(mock_docx_loader, mock_text_loader, mock_pdf_reader):
    ...
    assert index_update.utils.extract_text("file.pdf") == "PDF content"
    assert index_update.utils.extract_text("file.md") == "Text content"
    assert index_update.utils.extract_text("file.docx") == "Docx content"
    assert index_update.utils.extract_text("file.txt") == "Text content"
    assert index_update.utils.extract_text("file.py") is None
...
@pytest.mark.parametrize(
    ...
    [
        (f"{os.sep}home{os.sep}user{os.sep}documents{os.sep}projects{os.sep}file.txt", "file.txt"),     # Typical case
        (f"{os.sep}a{os.sep}b{os.sep}c{os.sep}d{os.sep}e{os.sep}f{os.sep}g{os.sep}h{os.sep}i.txt", f"e{os.sep}f{os.sep}g{os.sep}h{os.sep}i.txt"),  # Path with exactly 6 segments
        (f"{os.sep}a{os.sep}b{os.sep}c{os.sep}d{os.sep}e.txt", "e.txt"),                                                                       # Path with fewer than 6 segments
        (f"C:{os.sep}Users{os.sep}User{os.sep}Documents{os.sep}file.txt", ""),                                  # Windows-style path
        (f"{os.sep}", ""),                                                                                                                # Root directory (no segments)
        ("", "")                                                                                                                          # Empty string
    ]
)
@pytest.mark.unit
def test_trim_path(input_path, expected_output):
    # Create a logger
    logger = logging.getLogger(__name__)
    logging.basicConfig(level=logging.INFO)

    logger.info(f"Asserting that the path was correctly trimmed -> trimmed path={index_update.utils.trim_path(input_path)} & expected path={expected_output}")
    assert index_update.utils.trim_path(input_path) == expected_output